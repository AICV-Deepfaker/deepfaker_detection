...
class Video(CreatedTimestampMixin, Base, table=True):
    ...
    sources: Source | None = Relationship(back_populates="video", cascade_delete=True)
    results: Result | None = Relationship(back_populates="video", cascade_delete=True)
...
class FastReport(ReportBase, table=True):
    ...
    freq_conf: float
    freq_image: str = Field(max_length=255)
    rppg_result: ResultEnum
    rppg_conf: float
    rppg_image: str = Field(max_length=255)
