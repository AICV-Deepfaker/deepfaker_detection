...
class CRUDFastReport(CRUDBase):
    # 사용 : FastReport 저장
    @classmethod
    def create(cls, db: Session, db_report: FastReport):
        """Fast 분석 리포트 생성"""
        db.add(db_report)
    ...
    @classmethod
    def get_by_result(cls, db: Session, result_id: UUID):
        """result_id로 Fast 리포트 조회"""
        query = select(FastReport).where(FastReport.result_id == result_id)
        return db.scalars(query).one_or_none()


class CRUDDeepReport(CRUDBase):
    # 사용 : DeepReport 저장
    @classmethod
    def create(cls, db: Session, db_report: DeepReport):
        """Deep 분석 리포트 생성"""
        db.add(db_report)
        cls.commit_or_flush(db)
        db.refresh(db_report)
